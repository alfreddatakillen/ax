...
lb config \
	-a amd64 \
	--bootappend-live "boot=live components keyboard-layouts=se live-config.hostname=machine live-config.timezone=Europe/Zurich live-config.user-default-groups=cdrom,floppy,sudo,audio,dip,video,plugdev,users,user,netdev,docker toram" \
